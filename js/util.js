...
export const util = (() => {
  ...
  const generate = () => {
    const name = new URLSearchParams(window.location.search).get("generate");
    const generate = document.getElementById("generate-name");
    ...
    const div = document.createElement("div");
    div.classList.add("m-2");
    div.innerHTML = `<div class="generateContainer">
        <form id="invitationForm">
            <input type="text" id="to" name="to" required placeholder="Masukkan Nama Tamu">
            <button type="submit">Generate message</button>
            <p id="copyMessage" style="display: none;">Message copied to clipboard!</p>
        </form>
    </div>`;
